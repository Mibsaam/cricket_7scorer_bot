...
def start_next_series_match():
    global match
    with STATE_LOCK:
        old = match
        next_match_num = old.get("series_current_match_num", 1) + 1
        total_matches = old.get("series_total_matches", 1)
        
        blank = create_blank_match()
        blank["series_list"] = copy.deepcopy(old.get("series_list", []))
        blank["grounds_list"] = copy.deepcopy(old.get("grounds_list", []))
        blank["squads"] = copy.deepcopy(old.get("squads", {}))
        blank["team_records"] = copy.deepcopy(old.get("team_records", {}))
        blank["h2h_records"] = copy.deepcopy(old.get("h2h_records", {}))
        blank["matchup_db"] = copy.deepcopy(old.get("matchup_db", {}))
        blank["career_db"] = copy.deepcopy(old.get("career_db", {}))
        blank["match_archives"] = copy.deepcopy(old.get("match_archives", []))
        
        blank["series_name"] = old.get("series_name", "Cricket Championship 2026")
        blank["series_total_matches"] = total_matches
        blank["series_current_match_num"] = next_match_num
        blank["series_tally"] = copy.deepcopy(old.get("series_tally", {}))
        blank["teams"] = copy.deepcopy(old.get("teams", ["Team A", "Team B"]))
        blank["batting_team"] = blank["teams"][0]
        blank["bowling_team"] = blank["teams"][1]
...
def ensure_player(p_name, team="General", username=None):
    ...
    if p_name not in match["career_db"]:
        ...
        if username and not match["career_db"][p_name].get("username"):
            match["career_db"][p_name]["username"] = username
...
def get_large_scoreboard_text():
    ...
    mom_display = ""
    if match["match_status"] in ["Finished", "Abandoned", "Cancelled"]:
        ...
        for inn_idx in [1, 2]:
            ...
            for p, b_st in i_data.get("bowling", {}).items():
                ...
                if pts > max_pts:
                    max_pts, best_p = pts, p
        mom_display = f"⭐ Man of the Match: **{clean_txt(best_p)}**\n"
...
def get_scorer_keyboard(uid):
    ...
    if match.get("match_status") in ["Finished", "Abandoned", "Cancelled"]:
        curr_m = match.get("series_current_match_num", 1)
        tot_m = match.get("series_total_matches", 1)
        if curr_m < tot_m:
            m.add(InlineKeyboardButton(f"➡️ Start Next Match in Series ({curr_m+1}/{tot_m})", callback_data="act_next_series_match"))
        m.add(
...
def validate_on_field_players(cid):
    if match["match_status"] == "Innings Break":
        safe_send_message(cid, "🏁 **Innings 1 Over ho chuki hai!** Niche diye gaye button se 2nd Innings shuru karein:", reply_markup=get_scorer_keyboard(ADMIN_ID))
        return False
...
def start_wizard_squad_step(cid, mid):
    ...
    m = InlineKeyboardMarkup(row_width=1)
    if match["career_db"]:
        m.add(InlineKeyboardButton("📂 Add from Existing Career Database", callback_data=f"wiz_existing_roster_{t1}"))
    ...
    txt = f"📌 **Step 4: Manage Squads for {t1} & {t2}:**\n(Aap purane database se select kar sakte hain ya naye naam type kar sakte hain)"
    if mid:
...
def master_action_handler(c):
    ...
    try:
        ...
        if dt.startswith("arc_"):
            ...
            if entry:
                scorecard_txt = generate_detailed_scorecard_text(entry)
                m = InlineKeyboardMarkup(row_width=1)
                m.add(InlineKeyboardButton("⬅️ Back to Archives", callback_data="view_archives"))
                return safe_edit_message(scorecard_txt, chat_id=cid, message_id=mid, reply_markup=m)
        ...
        if dt == "view_mom":
            ...
            series_len = match.get("series_total_matches", 1)
            curr_match_num = match.get("series_current_match_num", 1)
            
            if series_len > 1 and curr_match_num >= series_len:
                mos_display = f"🎖️ **SERIES MVP (Man of the Series):**\n👉 `{clean_txt(mos_p)}` (Series Index: `{mos_pts:.1f}` pts)\n"
            elif series_len > 1:
                mos_display = f"🎖️ **Man of the Series:** *(Will be awarded after Match {series_len} of {series_len})*\n"
            else:
                mos_display = "🎖️ **Series Award:** *(Standalone Single Match - No Series MVP)*\n"

            res_txt = (
                f"⭐ **MATCH IMPACT LEADER (Man of the Match):**\n"
                f"👉 `{clean_txt(best_p)}` (Impact Score: `{max_pts:.1f}` pts)\n\n"
                f"{mos_display}\n"
                f"🏆 **Series Format:** {series_len}-Match Series (Current: Match {curr_match_num})\n"
                f"📊 *Calculated dynamically across active match archives & records!*"
            )
            m = InlineKeyboardMarkup(row_width=1)
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
            return safe_edit_message(res_txt, chat_id=cid, message_id=mid, reply_markup=m)

        if dt == "act_next_series_match":
            start_next_series_match()
            m = InlineKeyboardMarkup(row_width=2)
            ...
            return safe_edit_message(f"🏆 **SERIES MATCH {match['series_current_match_num']}/{match['series_total_matches']}**\nTeams: **{match['teams'][0]} vs {match['teams'][1]}**\n\n📌 **Toss Setup for Next Match:**", chat_id=cid, message_id=mid, reply_markup=m)

        if dt.startswith("wiz_existing_roster_"):
            target_t = dt.replace("wiz_existing_roster_", "")
            m = InlineKeyboardMarkup(row_width=2)
            for p_name in match["career_db"].keys():
                if p_name not in match["squads"].get(target_t, []):
                    m.add(InlineKeyboardButton(f"➕ {p_name}", callback_data=f"wiz_pick_exist_{target_t}_{p_name}"))
            m.add(InlineKeyboardButton("⬅️ Back to Squad Menu", callback_data=f"wiz_back_squad_{target_t}"))
            return safe_edit_message(f"📂 **Select Existing Player for {target_t}:**", chat_id=cid, message_id=mid, reply_markup=m)

        if dt.startswith("wiz_pick_exist_"):
            parts = dt.split("_", 4)
            target_t, p_name = parts[3], parts[4]
            if target_t not in match["squads"]: match["squads"][target_t] = []
            if p_name not in match["squads"][target_t]:
                match["squads"][target_t].append(p_name)
            save_data()
            return start_wizard_squad_step(cid, mid)

        if dt.startswith("wiz_back_squad_"):
            return start_wizard_squad_step(cid, mid)

        # Telegram Username assignment trigger menu
        if dt == "menu_set_telegram_handle":
            m = InlineKeyboardMarkup(row_width=2)
            for p in match["career_db"].keys():
                m.add(InlineKeyboardButton(f"👤 {p}", callback_data=f"set_uname_{p}"))
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
            return safe_edit_message("✍️ Kis player ke sath **Telegram Username (@handle)** link karna hai?", chat_id=cid, message_id=mid, reply_markup=m)

        if dt.startswith("set_uname_"):
            p_name = dt.replace("set_uname_", "")
            match["temp_data"]["target_profile_rename"] = p_name
            match["user_actions"][uid] = "input_telegram_handle_val"
            m = InlineKeyboardMarkup(row_width=1)
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="menu_set_telegram_handle"))
            return safe_edit_message(f"✍️ **{p_name}** ka Telegram username type karke send karein (e.g. @username):", chat_id=cid, message_id=mid, reply_markup=m)

        # Merge Duplicate Profiles Menu Trigger
        if dt == "menu_merge_duplicate_profiles":
            match["user_actions"][uid] = "input_merge_profile_names"
            m = InlineKeyboardMarkup(row_width=1)
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
            return safe_edit_message("✍️ Do player profiles ko merge karne ke liye unke naam `OldName | NewName` format me type karke bhejein (jaise: `Adip | Adip Khan`):", chat_id=cid, message_id=mid, reply_markup=m)
        ...
        if dt == "type_custom_runs_val":
            ...
            m = InlineKeyboardMarkup(row_width=1)
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
            return safe_edit_message("✍️ Kitne runs bane type karke send karein:", chat_id=cid, message_id=mid, reply_markup=m)
        ...
        if dt == "type_catch_fielder":
            ...
            m = InlineKeyboardMarkup(row_width=1)
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
            return safe_edit_message(f"✍️ Catch lene wale **Fielder** ka naam type karke send karein:", chat_id=cid, message_id=mid, reply_markup=m)
        ...
        if dt in ["type_replace_str", "type_replace_nstr"]:
            ...
            m = InlineKeyboardMarkup(row_width=1)
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
            return safe_edit_message(f"✍️ Naye **{role_target}** ka naam type karke send karein:", chat_id=cid, message_id=mid, reply_markup=m)
        ...
        if dt.startswith("q_wkt_"):
            ...
            m = InlineKeyboardMarkup(row_width=1)
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
            return safe_edit_message("✍️ Opponent ke total **Runs** type karke bhejein:", chat_id=cid, message_id=mid, reply_markup=m)
        ...
        if dt == "ext_set_custom":
            ...
            m = InlineKeyboardMarkup(row_width=1)
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
            return safe_edit_message("✍️ Naya Total Matches count type karein:", chat_id=cid, message_id=mid, reply_markup=m)
        ...
        if dt == "menu_squads_master":
            ...
            m.add(InlineKeyboardButton("🔗 Link Telegram Handle", callback_data="menu_set_telegram_handle"))
            m.add(InlineKeyboardButton("🔀 Merge Duplicate Profiles", callback_data="menu_merge_duplicate_profiles"))
        ...
        if dt.startswith("single_p_add_"):
            ...
            m = InlineKeyboardMarkup(row_width=1)
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="menu_squads_master"))
            return safe_edit_message(f"✍️ **{team_n}** ke single player ka naam type karke send karein:", chat_id=cid, message_id=mid, reply_markup=m)
        ...
        if dt.startswith("ren_p_"):
            ...
            m = InlineKeyboardMarkup(row_width=1)
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="menu_squads_master"))
            return safe_edit_message(f"✍️ **{p_name}** ka naya naam type karke send karein:", chat_id=cid, message_id=mid, reply_markup=m)
        ...
        if dt.startswith("ren_t_"):
            ...
            m = InlineKeyboardMarkup(row_width=1)
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="menu_squads_master"))
            return safe_edit_message(f"✍️ **{t_name}** ka naya naam type karke send karein:", chat_id=cid, message_id=mid, reply_markup=m)
        ...
        if dt == "pop_add_scorer":
            ...
            m = InlineKeyboardMarkup(row_width=1)
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
            return safe_edit_message("✍️ Naye Scorer ki **Telegram User ID** type karke bhejein:", chat_id=cid, message_id=mid, reply_markup=m)
        ...
        if dt == "edit_tot_runs":
            ...
            m = InlineKeyboardMarkup(row_width=1)
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
            return safe_edit_message("✍️ Sahi **Total Runs** type karke send karein:", chat_id=cid, message_id=mid, reply_markup=m)

        if dt == "edit_tot_wkts":
            match["user_actions"][uid] = "input_edit_wkts"
            m = InlineKeyboardMarkup(row_width=1)
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
            return safe_edit_message("✍️ Sahi **Total Wickets** type karke send karein:", chat_id=cid, message_id=mid, reply_markup=m)

        if dt == "edit_tot_target":
            match["user_actions"][uid] = "input_edit_target"
            m = InlineKeyboardMarkup(row_width=1)
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
            return safe_edit_message("✍️ Naya **Target** type karke send karein:", chat_id=cid, message_id=mid, reply_markup=m)
        ...
        if dt == "dls_inn1_cut":
            ...
            m = InlineKeyboardMarkup(row_width=1)
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
            return safe_edit_message(f"🌧️ **Inning 1 Interruption:** Match originally {match['original_match_overs']} overs ka tha.\nAb revised **Total Overs** kitne hue? (e.g. 2, 5):", chat_id=cid, message_id=mid, reply_markup=m)

        if dt == "dls_inn2_cut":
            match["user_actions"][uid] = "input_dls_inn2_revised"
            m = InlineKeyboardMarkup(row_width=1)
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
            return safe_edit_message("🌧️ **Inning 2 Target Revision:** 2nd Inning ab total kitne overs ki hogi? (e.g. 5):", chat_id=cid, message_id=mid, reply_markup=m)
    ...
    try:
        if act == "wiz_type_series_name":
            ...
            for name in parsed_names:
                ...
                if c_name:
                    if c_name in match["career_db"] and c_name not in match["squads"][t_target]:
                        match["squads"][t_target].append(c_name)
                    elif c_name in match["squads"][t_target]:
                        duplicates_found.append(c_name)
                    else:
                        match["squads"][t_target].append(c_name)
                        ensure_player(c_name, t_target)
            save_data()
            
            dup_msg = f"\n⚠️ Note: {', '.join(duplicates_found)} pehle se squad me hain." if duplicates_found else ""
            m = InlineKeyboardMarkup(row_width=2)
            if match["career_db"]:
                m.add(InlineKeyboardButton("📂 Add Existing Player", callback_data=f"wiz_existing_roster_{t_target}"))
            m.add(InlineKeyboardButton(f"➕ Add More ({t_target})", callback_data=f"wiz_add_p_{t_target}"))
            m.add(InlineKeyboardButton("➡️ Continue to Ground & Overs", callback_data="wiz_ground_step"))
            safe_send_message(msg.chat.id, f"✅ Squad updated for **{t_target}**!{dup_msg}", reply_markup=m)

        elif act == "input_single_player":
            t_target = match["temp_data"].get("target_team", match["batting_team"])
            if t_target not in match["squads"]: match["squads"][t_target] = []
            if txt in match["career_db"] and txt not in match["squads"][t_target]:
                match["squads"][t_target].append(txt)
                save_data()
                safe_send_message(msg.chat.id, f"✅ Existing player **{txt}** added to **{t_target}**!", reply_markup=get_scorer_keyboard(uid))
            elif txt not in match["squads"][t_target]:
                match["squads"][t_target].append(txt)
                ensure_player(txt, t_target)
                save_data()
                safe_send_message(msg.chat.id, f"✅ New player **{txt}** created & added to **{t_target}**!", reply_markup=get_scorer_keyboard(uid))
            else:
                safe_send_message(msg.chat.id, f"⚠️ **{txt}** pehle se is squad me maujud hai!", reply_markup=get_scorer_keyboard(uid))

        elif act == "input_merge_profile_names":
            parts = [p.strip() for p in raw_text.replace("|", ",").split(",") if p.strip()]
            if len(parts) >= 2:
                p1, p2 = clean_txt(parts[0]), clean_txt(parts[1])
                if p1 in match["career_db"] and p2 in match["career_db"]:
                    d1, d2 = match["career_db"][p1], match["career_db"][p2]
                    d1["runs"] += d2["runs"]
                    d1["balls"] += d2["balls"]
                    d1["fours"] += d2["fours"]
                    d1["sixes"] += d2["sixes"]
                    d1["wickets"] += d2["wickets"]
                    d1["bowled_balls"] += d2["bowled_balls"]
                    d1["runs_given"] += d2["runs_given"]
                    d1["catches"] += d2["catches"]
                    d1["drops"] += d2["drops"]
                    d1["stumpings"] = d1.get("stumpings", 0) + d2.get("stumpings", 0)
                    
                    del match["career_db"][p2]
                    save_data()
                    safe_send_message(msg.chat.id, f"✅ Success! **{p2}** ki profile ko **{p1}** ke sath successfully merge kar diya gaya hai.", reply_markup=get_scorer_keyboard(uid))
                else:
                    safe_send_message(msg.chat.id, "❌ Dono me se koi ek ya dono player database me nahi mile! Sahi naam type karein.", reply_markup=get_scorer_keyboard(uid))
            else:
                safe_send_message(msg.chat.id, "❌ Invalid format! Use: `OldName | NewName`", reply_markup=get_scorer_keyboard(uid))
        ...
        elif act == "input_telegram_handle_val":
            p_name = match["temp_data"].get("target_profile_rename")
            if p_name in match["career_db"]:
                match["career_db"][p_name]["username"] = txt.replace("@", "")
                save_data()
                safe_send_message(msg.chat.id, f"✅ Telegram Handle **@{match['career_db'][p_name]['username']}** successfully linked to **{p_name}**!", reply_markup=get_scorer_keyboard(uid))
        ...
        try:
            if match.get("timer_enabled") and match.get("timer_start_epoch") and not match.get("timer_paused") and match.get("match_status") == "Active":
                ...
                if not alerts.get("expired") and rem_sec <= 0:
                    alerts["expired"] = True
                    broadcast_commentary(cid, f"🚨 **TIME EXPIRED:** Over-rate penalties can now be applied.")
                    save_data()
        except Exception as e:
