...
def get_scorer_keyboard(uid):
    ...
    if match.get("match_status") in ["Finished", "Abandoned", "Cancelled"]:
        ...
        m.add(InlineKeyboardButton("🚀 Start Next Match / Reset", callback_data="act_reset_stats_confirm"))
        return m
...
def master_action_handler(c):
    ...
    try:
        ...
        if dt == "dls_inn1_cut":
            ...
            return safe_edit_message("🌧️ **Inning 1 Interruption:** Match ab kitne overs ka hoga? (e.g. 5):", chat_id=cid, message_id=mid)

        if dt == "dls_inn2_cut":
            match["user_actions"][uid] = "input_dls_inn2_revised"
            return safe_edit_message("🌧️ **Inning 2 Target Revision:** 2nd Inning ab kitne overs ki hogi? (e.g. 5):", chat_id=cid, message_id=mid)
        ...
        if dt == "act_reset_stats_exec":
            ...
            blank["teams"] = match.get("teams", ["Team A", "Team B"])
            blank["ground"] = match.get("ground", "Local Arena")
            blank["series_name"] = match.get("series_name", "Cricket Championship 2026")
            match.clear()
            match.update(blank)
            save_data()
            return safe_edit_message(get_large_scoreboard_text(), chat_id=cid, message_id=mid, reply_markup=get_scorer_keyboard(uid))
    ...
    try:
        if act == "wiz_type_series_name":
            ...
            try:
                ...
                save_data()
                sync_pinned_card(msg.chat.id)
                safe_send_message(msg.chat.id, f"✅ **DLS Applied!** Match Overs: `{rev_ov}.0`\n\n{get_large_scoreboard_text()}", reply_markup=get_scorer_keyboard(uid))
            except Exception:
