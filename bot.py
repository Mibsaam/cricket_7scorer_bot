...
def create_blank_match():
    return {
        ...
        "series_name": "Cricket Tournament 2026",
        "ground": "Local Arena",
        "grounds_list": ["Local Arena"],
        "stage": "League Match",
        "current_inning": 1,
        "teams": ["Team A", "Team B"],
        "batting_team": "Team A",
...
def master_action_handler(c):
    try:
        ...
        # ================= ADVANCED SQUADS & TEAMS MANAGEMENT (COMPLETE CRUD) =================
        if dt == "menu_squads_master":
            m = InlineKeyboardMarkup(row_width=2)
            m.add(InlineKeyboardButton("➕ Create New Team", callback_data="team_crud_create"))
            m.add(InlineKeyboardButton("🔄 Select Active Match Teams", callback_data="team_crud_select_active"))
            m.add(InlineKeyboardButton("✏️ Rename Team", callback_data="team_crud_rename_menu"))
            m.add(InlineKeyboardButton("🗑️ Delete Team", callback_data="team_crud_delete_menu"))
            m.add(InlineKeyboardButton("👥 Manage Players in Squad", callback_data="player_crud_main_menu"))
            m.add(InlineKeyboardButton("📋 View All Teams & Squads", callback_data="team_crud_view_all"))
            m.add(InlineKeyboardButton("⬅️ Back to Scoreboard", callback_data="back_main"))
            return bot.edit_message_text("👥 **TEAMS & SQUADS MASTER PANEL:**\nChunein kya manage karna hai:", chat_id=cid, message_id=c.message.message_id, reply_markup=m, parse_mode="Markdown")

        if dt == "team_crud_create":
            match["awaiting_action"] = "input_create_new_team"
        ...
        if dt == "team_crud_select_active":
            m = InlineKeyboardMarkup(row_width=2)
            for t in match.get("teams", []):
                m.add(InlineKeyboardButton(f"🔴 Bat: {t}", callback_data=f"set_bat_team_{t}"), InlineKeyboardButton(f"🟢 Bowl: {t}", callback_data=f"set_bowl_team_{t}"))
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="menu_squads_master"))
            return bot.edit_message_text("⚔️ **Select Current Playing Teams:**", chat_id=cid, message_id=c.message.message_id, reply_markup=m, parse_mode="Markdown")

        if dt.startswith("set_bat_team_"):
            t_name = dt.replace("set_bat_team_", "")
            match["batting_team"] = t_name
            save_data()
            return bot.edit_message_text(f"✅ Batting Team set to **{t_name}**!\n\n{get_large_scoreboard_text()}", chat_id=cid, message_id=c.message.message_id, reply_markup=get_scorer_keyboard(uid), parse_mode="Markdown")

        if dt.startswith("set_bowl_team_"):
            t_name = dt.replace("set_bowl_team_", "")
            match["bowling_team"] = t_name
            save_data()
            return bot.edit_message_text(f"✅ Bowling Team set to **{t_name}**!\n\n{get_large_scoreboard_text()}", chat_id=cid, message_id=c.message.message_id, reply_markup=get_scorer_keyboard(uid), parse_mode="Markdown")

        if dt == "team_crud_rename_menu":
            m = InlineKeyboardMarkup(row_width=2)
            for t in match.get("teams", []):
                m.add(InlineKeyboardButton(f"✏️ {t}", callback_data=f"rename_team_sel_{t}"))
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="menu_squads_master"))
            return bot.edit_message_text("✏️ Kis team ka naam badalna chahte hain?", chat_id=cid, message_id=c.message.message_id, reply_markup=m, parse_mode="Markdown")

        if dt.startswith("rename_team_sel_"):
            t_old = dt.replace("rename_team_sel_", "")
            match["temp_data"]["rename_old_team"] = t_old
            match["awaiting_action"] = "input_rename_team_value"
            return bot.edit_message_text(f"✍️ Team **{t_old}** ka naya naam type karein:", chat_id=cid, message_id=c.message.message_id, parse_mode="Markdown")

        if dt == "team_crud_delete_menu":
            m = InlineKeyboardMarkup(row_width=2)
            for t in match.get("teams", []):
                m.add(InlineKeyboardButton(f"🗑️ Delete {t}", callback_data=f"delete_team_confirm_{t}"))
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="menu_squads_master"))
            return bot.edit_message_text("🗑️ **Select Team to Delete Permanently:**", chat_id=cid, message_id=c.message.message_id, reply_markup=m, parse_mode="Markdown")

        if dt.startswith("delete_team_confirm_"):
            t_del = dt.replace("delete_team_confirm_", "")
            if len(match.get("teams", [])) <= 2:
                return bot.send_message(cid, "⚠️ Kam se kam 2 teams match ke liye honi chahiye, delete nahi ho sakti!")
            if t_del in match["teams"]: match["teams"].remove(t_del)
            if t_del in match["squads"]: del match["squads"][t_del]
            if match["batting_team"] == t_del: match["batting_team"] = match["teams"][0]
            if match["bowling_team"] == t_del: match["bowling_team"] = match["teams"][1]
            save_data()
            return bot.edit_message_text(f"✅ Team **{t_del}** aur uska squad delete ho gaya!", chat_id=cid, message_id=c.message.message_id, reply_markup=get_scorer_keyboard(uid), parse_mode="Markdown")

        if dt == "team_crud_view_all":
            res = "👥 **FULL TOURNAMENT SQUADS DIRECTORY:**\n━━━━━━━━━━━━━━━━━━━━\n"
            for t in match.get("teams", []):
                sq = ", ".join(match["squads"].get(t, [])) or "No players added yet"
                res += f"📁 **{t}:**\n`{sq}`\n\n"
            m = InlineKeyboardMarkup().add(InlineKeyboardButton("⬅️ Back", callback_data="menu_squads_master"))
            return bot.edit_message_text(res, chat_id=cid, message_id=c.message.message_id, reply_markup=m, parse_mode="Markdown")

        # PLAYER CRUD
        if dt == "player_crud_main_menu":
            m = InlineKeyboardMarkup(row_width=2)
            for t in match.get("teams", []):
                m.add(InlineKeyboardButton(f"📁 Squad: {t}", callback_data=f"sel_team_for_players_{t}"))
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="menu_squads_master"))
            return bot.edit_message_text("📁 Kis team ke players ko Add / Edit / Remove karna hai?", chat_id=cid, message_id=c.message.message_id, reply_markup=m, parse_mode="Markdown")

        if dt.startswith("sel_team_for_players_"):
            t_name = dt.replace("sel_team_for_players_", "")
            match["temp_data"]["active_crud_team"] = t_name
            m = InlineKeyboardMarkup(row_width=2)
            m.add(InlineKeyboardButton("➕ Add New Player", callback_data="ply_action_add"))
            m.add(InlineKeyboardButton("✏️ Rename Player", callback_data="ply_action_rename_list"))
            m.add(InlineKeyboardButton("🗑️ Remove Player", callback_data="ply_action_remove_list"))
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="player_crud_main_menu"))
            return bot.edit_message_text(f"👥 Manage Players for **{t_name}**:", chat_id=cid, message_id=c.message.message_id, reply_markup=m, parse_mode="Markdown")

        if dt == "ply_action_add":
            t_name = match["temp_data"].get("active_crud_team", match["batting_team"])
            match["awaiting_action"] = "input_add_player_to_team"
            return bot.edit_message_text(f"✍️ **{t_name}** ke naye player ka naam type karke bhejein:", chat_id=cid, message_id=c.message.message_id, parse_mode="Markdown")

        if dt == "ply_action_rename_list":
            t_name = match["temp_data"].get("active_crud_team", match["batting_team"])
            m = InlineKeyboardMarkup(row_width=2)
            for p in match["squads"].get(t_name, []):
                m.add(InlineKeyboardButton(f"✏️ {p}", callback_data=f"rename_ply_sel_{p}"))
            m.add(InlineKeyboardButton("⬅️ Back", callback_data=f"sel_team_for_players_{t_name}"))
            return bot.edit_message_text(f"✏️ Kis player ka naam badalna hai ({t_name})?", chat_id=cid, message_id=c.message.message_id, reply_markup=m, parse_mode="Markdown")

        if dt.startswith("rename_ply_sel_"):
            p_old = dt.replace("rename_ply_sel_", "")
            match["temp_data"]["rename_old_player"] = p_old
            match["awaiting_action"] = "input_rename_player_value"
            return bot.edit_message_text(f"✍️ Player **{p_old}** ka naya naam type karein:", chat_id=cid, message_id=c.message.message_id, parse_mode="Markdown")

        if dt == "ply_action_remove_list":
            t_name = match["temp_data"].get("active_crud_team", match["batting_team"])
            m = InlineKeyboardMarkup(row_width=2)
            for p in match["squads"].get(t_name, []):
                m.add(InlineKeyboardButton(f"🗑️ Remove {p}", callback_data=f"remove_ply_sel_{p}"))
            m.add(InlineKeyboardButton("⬅️ Back", callback_data=f"sel_team_for_players_{t_name}"))
            return bot.edit_message_text(f"🗑️ Kis player ko squad se hatana hai ({t_name})?", chat_id=cid, message_id=c.message.message_id, reply_markup=m, parse_mode="Markdown")

        if dt.startswith("remove_ply_sel_"):
            p_rem = dt.replace("remove_ply_sel_", "")
            t_name = match["temp_data"].get("active_crud_team", match["batting_team"])
            if p_rem in match["squads"].get(t_name, []):
                match["squads"][t_name].remove(p_rem)
            if match["striker"] == p_rem: match["striker"] = "Select Striker"
            if match["non_striker"] == p_rem: match["non_striker"] = "Select Non-Striker"
            if match["bowler"] == p_rem: match["bowler"] = "Select Bowler"
            save_data()
            return bot.edit_message_text(f"✅ Player **{p_rem}** squad se remove ho gaya!", chat_id=cid, message_id=c.message.message_id, reply_markup=get_scorer_keyboard(uid), parse_mode="Markdown")
        ...
        if dt.startswith("drop_by_"):
            ...
            broadcast_commentary(cid, f"😱 💔 **CATCH DROPPED!** `{f_name}` drops a catch off `{match['bowler']}`!")
            save_data()
            return bot.edit_message_text(get_large_scoreboard_text(), chat_id=cid, message_id=c.message.message_id, reply_markup=get_scorer_keyboard(uid), parse_mode="Markdown")

        # 8. NON-STRIKER SELECTION
        ...
        # 11. QUICK MANUAL OPPONENT INNINGS
        ...
        if dt == "menu_toss":
            if len(match.get("teams", [])) < 2:
                return bot.send_message(cid, "⚠️ Pehle Squad menu se kam se kam 2 teams banayein!")
...
# ================= TEXT INPUT HANDLER (FULL CRUD LINKED) =================
...
def handle_master_text_inputs(msg):
    ...
    # 1. TEAM CREATION
    if act == "input_create_new_team":
        if txt not in match["teams"]:
            match["teams"].append(txt)
            match["squads"][txt] = []
            save_data()
            bot.reply_to(msg, f"✅ Nayi Team **{txt}** safalta se create ho gayi!", reply_markup=get_scorer_keyboard(uid), parse_mode="Markdown")
        else:
            bot.reply_to(msg, "⚠️ Yeh team pehle se exist karti hai!")

    # 2. TEAM RENAME
    elif act == "input_rename_team_value":
        old_t = match["temp_data"].get("rename_old_team")
        if old_t and old_t in match["teams"]:
            idx = match["teams"].index(old_t)
            match["teams"][idx] = txt
            match["squads"][txt] = match["squads"].pop(old_t, [])
            if match["batting_team"] == old_t: match["batting_team"] = txt
            if match["bowling_team"] == old_t: match["bowling_team"] = txt
            save_data()
            bot.reply_to(msg, f"✅ Team **{old_t}** ka naam badal kar **{txt}** kar diya gaya!", reply_markup=get_scorer_keyboard(uid), parse_mode="Markdown")

    # 3. PLAYER ADD
    elif act == "input_add_player_to_team":
        t_name = match["temp_data"].get("active_crud_team", match["batting_team"])
        ensure_player(txt, t_name)
        if txt not in match["squads"].get(t_name, []):
            if t_name not in match["squads"]: match["squads"][t_name] = []
            match["squads"][t_name].append(txt)
            save_data()
            bot.reply_to(msg, f"✅ Player **{txt}** team **{t_name}** mein add ho gaya!", reply_markup=get_scorer_keyboard(uid), parse_mode="Markdown")
        else:
            bot.reply_to(msg, "⚠️ Yeh player pehle se is squad mein hai!")

    # 4. PLAYER RENAME
    elif act == "input_rename_player_value":
        old_p = match["temp_data"].get("rename_old_player")
        t_name = match["temp_data"].get("active_crud_team", match["batting_team"])
        if old_p and old_p in match["squads"].get(t_name, []):
            idx = match["squads"][t_name].index(old_p)
            match["squads"][t_name][idx] = txt
            if old_p in match["career_db"]:
                match["career_db"][txt] = match["career_db"].pop(old_p)
            ...
            save_data()
            bot.reply_to(msg, f"✅ Player **{old_p}** ka naam update hoke **{txt}** ho gaya!", reply_markup=get_scorer_keyboard(uid), parse_mode="Markdown")

    # 5. OVER SETTINGS
    elif act == "input_manual_overs":
        try:
            ov = int(txt)
    ...
    # 8. DIRECT STRIKER / NON-STRIKER / BOWLER CREATION
