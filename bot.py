...
def master_action_handler(c):
    ...
    try:
        ...
        if dt == "view_mom":
            ...
            series_info_txt = f"🏆 **Series Format:** {series_len}-Match Series" if series_len > 1 else "🏆 **Standalone Single Match**"

            res_txt = (
                f"⭐ **MATCH IMPACT LEADER (Man of the Match):**\n"
                f"👉 `{clean_txt(best_p)}` (Impact Score: `{max_pts:.1f}` pts)\n\n"
                f"🎖️ **SERIES MVP (Man of the Series):**\n"
                f"👉 `{clean_txt(mos_p)}` (Series Index: `{mos_pts:.1f}` pts)\n\n"
                f"{series_info_txt}\n"
