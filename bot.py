...
def start_next_series_match():
    """Start the next match in a multi-match series while preserving teams, squads, and series state."""
    global match
    with STATE_LOCK:
        old = match
...
def master_action_handler(c):
    ...
    try:
        ...
        if dt.startswith("set_uname_"):
            ...
            return safe_edit_message(f"✍️ **{p_name}** ka Telegram username type karke send karein (e.g. @username):", chat_id=cid, message_id=mid)

        # Merge Duplicate Profiles Menu Trigger
        if dt == "menu_merge_duplicate_profiles":
            match["user_actions"][uid] = "input_merge_profile_names"
            return safe_edit_source_text = "✍️ Do player profiles ko merge karne ke liye unke naam `OldName | NewName` format me type karke bhejein (jaise: `Adip | Adip Khan`):"
            return safe_edit_message(safe_edit_source_text, chat_id=cid, message_id=mid)
            ...
            for name in parsed_names:
                ...
                if c_name:
                    if c_name in match["career_db"] and c_name not in match["squads"][t_target]:
                        # Already exists in career database, link cleanly
