...
def calculate_allocated_time_mins(overs):
    # Standard formula ~4.25 mins per over
    ov_time_map = {
...
def get_large_scoreboard_text():
    ...
    return (
        ...
        f"{timer_str}🎙️ Event: {clean_ticker}"
    )
...
def get_available_batsmen(team_name):
    ...
    for p in match["squads"].get(team_name, []):
        ...
        if p_clean.lower() in current_active:
            continue
            
        matched_entry = None
        for b_name, b_stat in b_dict.items():
            if clean_txt(b_name).lower() == p_clean.lower():
                matched_entry = b_stat
                break
                
        if matched_entry:
            status = matched_entry.get("status", "Not Out")
...
def process_wicket(cid, mid, reason, uid, target_batter="striker"):
    ...
    if match["wickets"] < limit_w:
        ...
        safe_send_message(cid, f"👤 **{clean_txt(dismissed)} OUT!** Next Batsman ({'Striker' if target_batter == 'striker' else 'Runner'}) select karein:", reply_markup=m)
        if mid:
            return safe_edit_message(get_large_scoreboard_text(), chat_id=cid, message_id=mid, reply_markup=get_scorer_keyboard(uid))
    ...
    try:
        if act == "wiz_type_series_name":
            ...
            for p in avail:
                m.add(InlineKeyboardButton(f"🏃 {p}", callback_data=f"wiz_nstr_{p}"))
            m.add(InlineKeyboardButton("➕ Type Non-Striker", callback_data="wiz_type_nonstriker"))
            safe_send_message(msg.chat.id, f"✅ Striker: **{txt}**\n\n📌 **Step 7.2:** Select/Type **Non-Striker (Runner):**", reply_markup=m)
            ...
            for p in match["squads"].get(match["bowling_team"], []):
                m.add(InlineKeyboardButton(f"⚾ {p}", callback_data=f"wiz_bowl_{p}"))
            m.add(InlineKeyboardButton("➕ Type Opening Bowler", callback_data="wiz_type_bowler"))
            safe_send_message(msg.chat.id, f"✅ Runner: **{txt}**\n\n📌 **Step 7.3:** Select/Type **Opening Bowler ({match['bowling_team']}):**", reply_markup=m)
        ...
        try:
            if match.get("timer_enabled") and match.get("timer_start_epoch") and not match.get("timer_paused") and match.get("match_status") == "Active":
                ...
                if not alerts.get("midway") and elapsed_sec >= (total_sec / 2):
                    alerts["midway"] = True
                    broadcast_commentary(cid, f"⏱️ **MIDWAY TIME CHECK:** Inning half-time complete! `{int(rem_sec // 60)} Mins` remaining for {match['total_match_overs']} overs.")
                    save_data()
                    
                # 2. 5-Minute Warning
                if not alerts.get("warn5") and rem_sec <= 300 and rem_sec > 0:
                    alerts["warn5"] = True
                    broadcast_commentary(cid, f"⚠️ **PACE WARNING:** Only `5 Minutes` left! Bowling team pace up karein.")
                    save_data()
                    
                # 3. Expired Alert
                if not alerts.get("expired") and rem_sec <= 0:
                    alerts["expired"] = True
                    broadcast_commentary(cid, "🚨 **TIME EXPIRED:** Inning time allocation complete! Over-rate penalties can now be applied from dashboard.")
                    save_data()
        except Exception as e:
