...
def create_blank_match():
    return {
        ...
        "bowling_team": "Team B",
        "total_match_overs": 7,
        "max_wickets_limit": 10,  # Dynamically configurable for Gully Cricket (up to 15+)
        "is_super_over": False,
        "toss_winner": None,
        "toss_decision": None,
        "runs": 0,
        ...
        "over_worm": {},
        "match_status": "Active",
        "user_actions": {},  # Per-user isolated action listener
        "temp_data": {},
        "history": [],
        "squads": {},
        "match_innings_data": {
...
def save_state_for_undo():
    match["history"].append({
        "match_state": copy.deepcopy({
            ...
            "match_innings_data": match["match_innings_data"],
...
def get_large_scoreboard_text():
    ...
    if match.get("current_inning") == 2:
        ...
        bottom_box = (
            f"╭──────────────────────────────────────────────────────╮\n"
            f"│  🎯 𝐓𝐀𝐑𝐆𝐄𝐓: {match['target']}  │  𝐍𝐞𝐞𝐝 {needed} 𝐢𝐧 {b_left} 𝐛𝐚𝐥𝐥𝐬\n"
            f"│  🎞️ 𝐁𝐀𝐋𝐋𝐒 : [ {rec_balls} ]\n"
            f"╰──────────────────────────────────────────────────────╯"
        )
    else:
        bottom_box = (
            f"╭──────────────────────────────────────────────────────╮\n"
            f"│  🤝 𝐏'𝐒𝐇𝐈𝐏 : {match['partnership_runs']} ({match['partnership_balls']}b) │ ⚡ 𝐄𝐗𝐓𝐑𝐀𝐒: {match['extras_total']} (Wd:{match['extras_wides']}, NB:{match['extras_noballs']})\n"
            f"│  🎞️ 𝐁𝐀𝐋𝐋𝐒 : [ {rec_balls} ]\n"
            f"╰──────────────────────────────────────────────────────╯"
        )
    ...
    return (
        f"╭──────────────────────────────────────────────────────╮\n"
        f"│  🔴 **{match['batting_team']}**  :  【  **{match['runs']}  /  {match['wickets']}**  】  │  **{match['overs']}** Ov  │  𝐂𝐑𝐑: **{crr:.2f}**   │\n"
        f"╰──────────────────────────────────────────────────────╯\n\n"
        f"  🏏 ★  **{match['striker']}***   ▶   **{st_b['runs']}** ₍{st_b_sub}₎   │  𝟒𝐬: {st_b['fours']}  𝟔𝐬: {st_b['sixes']}\n\n"
        f"  🏃    **{match['non_striker']}**        ▶   **{nst_b['runs']}** ₍{nst_b_sub}₎   │  𝟒𝐬: {nst_b['fours']}  𝟔𝐬: {nst_b['sixes']}\n\n"
        f"  ⚾    **{match['bowler']}**      ▶   **{b_st['wickets']} / {b_st['runs']}** ({b_ov} ov)\n\n"
        f"{bottom_box}\n"
        f"🏟️ Ground: `{match['ground']}` │ 🏆 **{match['series_name']}** (M {match['series_current_match_num']}/{match['series_total_matches']}){fh_tag}\n"
        f"🎙️ Event: _{match['last_event_ticker']}_"
    )
...
def generate_detailed_scorecard_text(match_data=None):
    ...
    out = (
        ...
        f"🏟️ Ground: `{d['ground']}` │ Match Overs: `{d['total_match_overs']}.0` (Max Wkts: {d.get('max_wickets_limit', 10)})\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
    )
    ...
    for inn in [1, 2]:
        ...
        for b, st in bowling_dict.items():
            ...
            out += f"• **{b}**: `{ov} ov` (M:{st.get('maidens',0)}) │ `{st['runs']} runs` │ `{st['wickets']} wkts` │ Econ: `{econ:.2f}`\n"
        ...
        if fow_list:
            out += f"📉 **Fall of Wickets:** {', '.join(fow_list)}\n"
        out += "────────────────────────────────────────\n"
...
def register_legal_ball(cid, legal=True, ball_tag="0", runs_on_ball=0):
    if legal:
        ...
        if match["bowler"] == "Select Bowler":
            match["bowler"] = f"Bowler {comp_ov + 1}"
        ...
        # OVER ROTATION LOGIC (Deterministic Strike Swap)
        if rem_b == 0 and match["balls"] > 0:
            ...
            # If runs on last ball was odd, swap was already done, so over completion leaves it as-is.
            # If runs was even, over completion rotates the strike!
            if runs_on_ball % 2 == 0:
...
def master_action_handler(c):
    try:
        ...
        if dt == "view_mom":
            ...
            # Real Match-Specific MoM Scoring Calculation
            ...
            res_txt = (
                f"⭐ **MATCH IMPACT LEADER (MoM):** `{best_p}` (Score: `{max_pts:.1f}` pts)\n"
                f"🏆 Calculated on current match performances!\n\n"
                f"🎖️ **TOURNAMENT / SERIES MVP (MoS):**\n"
                f"👉 `{mos_p}` with `{mos_pts:.1f}` career performance index!"
            )
        ...
        if match.get("match_status") == "Finished" and (dt.startswith("act_run_") or dt.startswith("wkt_")):
        ...
        if dt.startswith("wiz_set_series_"):
            s_name = dt.replace("wiz_set_series_", "")
        ...
        if dt.startswith("wiz_set_ov_"):
            ...
            m = InlineKeyboardMarkup(row_width=3)
            m.add(InlineKeyboardButton("10 Wickets (Standard)", callback_data="wiz_set_wlimit_10"))
            m.add(InlineKeyboardButton("11 Wickets (Gully)", callback_data="wiz_set_wlimit_11"))
            m.add(InlineKeyboardButton("14 Wickets (Gully Mega)", callback_data="wiz_set_wlimit_14"))
            m.add(InlineKeyboardButton("✍️ Custom Wickets Limit", callback_data="wiz_type_wlimit"))
            return safe_edit_message(f"⏳ Match Overs: **{ov}.0**\n\n📌 **Step 6.2:** Select **Max Wickets Limit (Gully Cricket Supported):**", chat_id=cid, message_id=mid, reply_markup=m)
        ...
        if dt == "menu_dls_reduction":
            match["user_actions"][uid] = "input_dls_revised_overs"
...
def process_wicket(cid, mid, reason, uid, target_batter="striker"):
    ...
    if match["wickets"] < limit_w:
        ...
        fow_txt = f"{match['runs']}/{match['wickets']} ({dismissed}, {match['overs']} ov)"
...
def handle_text_inputs(msg):
    ...
    if act == "wiz_type_series_name":
        ...
        m.add(InlineKeyboardButton("🏆 3-Match Series", callback_data="wiz_series_3"), InlineKeyboardButton("🏆 5-Match Series", callback_data="wiz_series_5"))
        m.add(InlineKeyboardButton("✍️ Custom Series Length", callback_data="wiz_series_custom"))
        bot.reply_to(msg, f"✅ Series Name locked to **{txt}**!\n\n📌 **Step 1.2:** Select **Series Format:**", reply_markup=m, parse_mode="Markdown")
        ...
        try:
            ...
            match["user_actions"][uid] = "input_max_wickets_limit"
            bot.reply_to(msg, "❌ Invalid number! Please enter valid wickets limit (e.g. 10, 14):")
        ...
        for ov in [5, 6, 7, 8, 10, 12, 15, 20]:
            m.add(InlineKeyboardButton(f"{ov} Overs", callback_data=f"wiz_set_ov_{ov}"))
        bot.reply_to(msg, f"🏟️ Ground set to **{txt}**!\n\n📌 **Step 6.1:** Select **Total Match Overs:**", reply_markup=m, parse_mode="Markdown")
        ...
        try:
            ...
            m.add(InlineKeyboardButton("14 Wickets (Gully Mega)", callback_data="wiz_set_wlimit_14"))
            m.add(InlineKeyboardButton("✍️ Custom Wickets Limit", callback_data="wiz_type_wlimit"))
            bot.reply_to(msg, f"⏳ Match Overs: **{ov}.0**\n\n📌 **Step 6.2:** Select **Max Wickets Limit:**", reply_markup=m, parse_mode="Markdown")
        except:
