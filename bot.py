...
def create_blank_match():
    return {
        ...
        "team_records": {},
        "match_innings_data": {
            1: {"team": "Team A", "batting": {}, "bowling": {}, "extras": {"w": 0, "nb": 0, "b": 0, "lb": 0, "total": 0}, "fow": [], "partnerships": []},
            2: {"team": "Team B", "batting": {}, "bowling": {}, "extras": {"w": 0, "nb": 0, "b": 0, "lb": 0, "total": 0}, "fow": [], "partnerships": []}
        },
        "career_db": {},
        "match_archives": []
    }
...
def ensure_team_record(team_name):
    ...
    if team_name not in match["team_records"]:
        match["team_records"][team_name] = {"played": 0, "won": 0, "lost": 0, "tied": 0}
...
def ensure_match_player_stat(p_name, team, role="bat"):
    ...
    if inn not in match["match_innings_data"]:
        match["match_innings_data"][inn] = {"team": team, "batting": {}, "bowling": {}, "extras": {"w": 0, "nb": 0, "b": 0, "lb": 0, "total": 0}, "fow": [], "partnerships": []}
...
# ================= COMPACT MOBILE SCOREBOARD (LAYOUT 3) =================
def get_large_scoreboard_text():
    ...
    if match.get("current_inning") == 2 and match["target"] > 0:
        ...
        bottom_box = (
            f"╭──────────────────────────────╮\n"
            f"│ 🎯 𝐓𝐀𝐑𝐆𝐄𝐓: {to_serif_bold_num(match['target'])} │ 𝐍𝐞𝐞𝐝 {to_serif_bold_num(needed)} ({to_serif_bold_num(b_left)}𝐛)\n"
            f"│ 🎞️ [ {rec_balls} ]\n"
            f"╰──────────────────────────────╯"
        )
    else:
    ...
    return (
        f"╭──────────────────────────────╮\n"
        f"│ 🔴 **{team_name}** : 【 {s_runs} / {s_wkts} 】\n"
        f"│ ⏳ {s_overs} 𝐎𝐯   │ 𝐂𝐑𝐑: {s_crr}\n"
        f"╰──────────────────────────────╯\n\n"
        ...
        f"🏆 **{clean_txt(match['series_name'])}** (M {match['series_current_match_num']}/{match['series_total_matches']}){fh_tag}\n"
        f"🎙️ Event: {clean_ticker}"
    )

# ================= DETAILED SCORECARD WITH ALL BATTERS, BOWLERS, P'SHIPS & FOW =================
...
# ================= HELPER TO FILTER AVAILABLE BATSMEN =================
def get_available_batsmen(team_name):
    ...
    for p in match["squads"].get(team_name, []):
        if p == match["striker"] or p == match["non_striker"]:
            continue
        if p in b_dict:
            status = b_dict[p].get("status", "Not Out")
            if status == "Retired Hurt":
                avail.append(p)
        else:
            avail.append(p)
    return avail
...
def handle_teams_ledger(msg):
    ...
    if not match.get("team_records"):
        ...
        for t, rec in match["team_records"].items():
            ...
            out += f"• **{clean_txt(t)}** ▶ Played: `{rec['played']}` │ Won: `{rec['won']}` │ Lost: `{rec['lost']}` │ Win%: `{win_pct:.1f}%`\n"
...
def check_match_completion(cid):
    ...
    # 1ST INNINGS COMPLETION (Strict Ball-Based & Wicket-Based Check)
...
def register_legal_ball(cid, legal=True, ball_tag="0", runs_on_ball=0):
    if legal:
        ...
        # PRECISE INNINGS OVER LIMIT (BALL BASED)
...
def master_action_handler(c):
    ...
    try:
        ...
        # WIDE & NO BALL (WITH PRECISE EXTRA-BASED STRIKE ROTATION)
        if dt == "menu_wide":
            m = InlineKeyboardMarkup(row_width=3)
        ...
        # BYES & LEG BYES (BOWLER ECONOMY CLEAN)
        if dt == "menu_byes":
            m = InlineKeyboardMarkup(row_width=3)
        ...
        if dt.startswith("exec_bye_") or dt.startswith("exec_lb_"):
            ...
            # Batsman faces ball, but runs not in batsman account
            ensure_match_player_stat(match["striker"], match["batting_team"], role="bat")
            match["match_innings_data"][match["current_inning"]]["batting"][match["striker"]]["balls"] += 1
        ...
        # INJURY SPLIT (RETIRED HURT VS PERMANENT)
        ...
        if dt in ["retire_str_hurt", "retire_nstr_hurt"]:
            ...
            m = InlineKeyboardMarkup(row_width=2)
            for p in avail:
                m.add(InlineKeyboardButton(f"🏏 {p}", callback_data=f"sel_str_{p}" if is_str else f"sel_nonstr_{p}"))
            m.add(InlineKeyboardButton("➕ Type New Batsman", callback_data="wiz_type_striker" if is_str else "wiz_type_nonstriker"))
        ...
        # SQUAD & TEAM MANAGEMENT (WITH BULK ADD, TRANSFER, RENAME & REMOVE)
        ...
        if dt == "view_team_ledger":
            ...
            if not match.get("team_records"):
                ...
                for t, rec in match["team_records"].items():
                    win_pct = (rec["won"] / rec["played"] * 100) if rec["played"] > 0 else 0.0
                    out += f"• **{clean_txt(t)}** ▶ P: `{rec['played']}` │ W: `{rec['won']}` │ L: `{rec['lost']}` │ Win%: `{win_pct:.1f}%`\n"
            m = InlineKeyboardMarkup(row_width=1)
        ...
        # MATCH RESET WITHOUT DELETING TEAMS OR SQUADS
        if dt == "act_reset_stats_confirm":
            ...
            return safe_edit_message("⚠️ **Kya aap current match score reset karna chahte hain?**\n_(Teams aur Squads delete nahi honge)_", chat_id=cid, message_id=mid, reply_markup=m)
...
def process_wicket(cid, mid, reason, uid, target_batter="striker"):
    ...
    if match["wickets"] < limit_w:
        ...
        # Record Partnership for scorecard
        ...
        if act == "wiz_type_series_name":
            ...
            try:
                ...
                match["match_innings_data"][1] = {
                    ...
                    "extras": {"w": 0, "nb": 0, "b": 0, "lb": 0, "total": 0}, "fow": [], "partnerships": []
                }
                match.update({
                    "runs": 0, "wickets": 0, "overs": 0.0, "balls": 0,
