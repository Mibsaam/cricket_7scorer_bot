...
BOT_TOKEN = "8812331993:AAGbpmCf5z4nBNG2RzmzkyavArKDGCNvMeY"
ADMIN_ID = 874225351
...
def master_action_handler(c):
    try:
        ...
        if dt == "exec_start_super_over":
            ...
            save_data()
            broadcast_commentary(cid, f"🦅 🔥 **SUPER OVER COMMENCED!** 1 Over Shootout!")
            return bot.edit_message_text(get_large_scoreboard_text(), chat_id=cid, message_id=c.message.message_id, reply_markup=get_scorer_keyboard(uid), parse_mode="Markdown")
        ...
        if dt.startswith("exec_nb_"):
            ...
            if match["free_hit_enabled"]:
                ...
                broadcast_commentary(cid, f"⚠️ 🚀 **NO BALL!** Free Hit on next ball!")
        ...
        if dt.startswith("split_to_"):
            ...
            broadcast_commentary(cid, f"🚑 **INJURY CHANGE:** `{prev_b}` walks off. `{new_b}` will finish the over!")
            save_data()
            return bot.edit_message_text(get_large_scoreboard_text(), chat_id=cid, message_id=c.message.message_id, reply_markup=get_scorer_keyboard(uid), parse_mode="Markdown")
