...
def validate_on_field_players(cid):
    if match["match_status"] == "Innings Break":
        safe_send_message(cid, "🏁 **Innings 1 Over ho chuki hai!** Niche diye gaye button se Innings 2 start karein:", reply_markup=get_scorer_keyboard(ADMIN_ID))
        return False
...
def master_action_handler(c):
    ...
    try:
        ...
        if dt.startswith("wiz_bowl_"):
            ...
            # Reset ball state only on match startup (balls == 0)
            if match["balls"] == 0:
                match["runs"] = 0
                match["wickets"] = 0
                match["overs"] = 0.0
...
def run_telegram_worker():
    ...
    print(">>> Pro Cricket Engine Running...")
    while True:
