...
# ULTIMATE TOURNAMENT & MATCH ENGINE
match = {
    "series_name": "Pro Cricket Championship",
...
def get_scorer_markup():
    ...
    # 1. Standard Bat Runs (1D, 2D, 3D, 4, 6)
    m.add(
        InlineKeyboardButton("🔴 0 Dot", callback_data="sc_0"),
        InlineKeyboardButton("🟢 1D (1 Run)", callback_data="sc_1"),
        InlineKeyboardButton("🔵 2D (2 Runs)", callback_data="sc_2")
    )
    m.add(
        InlineKeyboardButton("🟡 3D (3 Runs)", callback_data="sc_3"),
...
def on_scorer_action(call):
    try:
        ...
        if dt == "menu_wk_captain":
            ...
            return bot.edit_message_text("🧤👑 **Wicketkeeper & Captain Setup:**\nSelect option:", chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=m, parse_mode="Markdown")
        ...
        # 4. WICKET / DISMISSAL MENU (Bowled, Caught, Run Out, Stumped)
        if dt == "menu_wickets":
            m = InlineKeyboardMarkup(row_width=2)
            m.add(
        ...
        if dt in ["sc_0", "sc_1", "sc_2", "sc_3", "sc_4", "sc_6"]:
            ...
            action_desc = f"Over {match['overs']}: {match['striker']} scored {r_val}D run(s) off {match['bowler']}"
...
if __name__ == "__main__":
    ...
    print("Pro Cricket Scorer with WK & Captain Selection is running 24/7...")
