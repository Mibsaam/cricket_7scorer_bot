...
BOT_TOKEN = "8812331993:AAHmi0s78veHmazGgw6G1a-85WzEVZAz1lQ"
ADMIN_ID = 874225351
...
# ULTIMATE TOURNAMENT & MATCH ENGINE WITH ALL 5 PRO FEATURES
match = {
    "series_name": "Ultimate Pro Championship 2026",
...
def get_scorer_markup():
    ...
    # 1. Standard Runs & Gully Rules (1, 2, 3, 4, 6 & 1D, 2D, 3D)
    m.add(
    ...
    m.add(
        InlineKeyboardButton("📌 1D", callback_data="sc_1d"),
        InlineKeyboardButton("📌 2D", callback_data="sc_2d"),
        InlineKeyboardButton("📌 3D", callback_data="sc_3d")
    )
    # 2. Extras & Wickets
    m.add(
    ...
    # 3. Squad, WK, Captain & Shot Tracker
    m.add(
    ...
    m.add(
        ...
        InlineKeyboardButton("🎯 Shot Area (Wagon)", callback_data="menu_shot_area")
    )
    m.add(
        InlineKeyboardButton("🧤 Set WK & Captain", callback_data="menu_wk_captain"),
        InlineKeyboardButton("👥 Manage Teams", callback_data="menu_manage_teams")
    )
    # 4. Pro Features (Super Over, DLS, Summary, Leaderboard)
    m.add(
        InlineKeyboardButton("🦅 Super Over Mode", callback_data="trigger_super_over"),
        InlineKeyboardButton("🌧️ DLS Target Calculator", callback_data="sc_dls")
    )
    m.add(
    ...
    m.add(
        ...
        InlineKeyboardButton("🗑️ Reset", callback_data="reset_practice")
    )
    m.add(
...
def get_scoreboard_text():
    ...
    # Feature 1: Target & Required Run Rate (RRR) Calculator for 2nd Innings
    target_info = ""
    if match["is_second_innings"]:
        runs_needed = match["target"] - match["runs"]
        balls_bowled = match["balls"]
        total_legal_balls = match["total_match_overs"] * 6
        balls_left = total_legal_balls - balls_bowled
        rrr = (runs_needed / (balls_left / 6)) if balls_left > 0 else 0.0
        target_info = f"\n 🎯 **Target:** `{match['target']}` | **Need:** `{runs_needed} runs` in `{balls_left} balls` (RRR: `{rrr:.2f}`)"
    ...
    so_info = " 🔥 [SUPER OVER ACTIVE]" if match["is_super_over"] else ""

    return (
        f"🏆 **{match['series_name']}**{so_info} 🏏\n"
        f"_{mode_info}_\n"
        f"──────────────────────────\n"
...
def c_start(msg):
    bot.reply_to(
        msg, 
        "⚡ **Ultimate Pro Cricket Scorer Initialized with All 5 Features!**\nControl panel active hai:", 
...
def on_scorer_action(call):
    try:
        ...
        # Feature 3: Super Over Mode Trigger
        if dt == "trigger_super_over":
            ...
            match["balls"] = 0
            match["extras"] = 0
            bot.answer_callback_query(call.id, "Super Over Activated!")
            return bot.edit_message_text("🔥 **SUPER OVER MODE ACTIVATED!** (1 Over, Max 2 Wickets)\n\n" + get_scoreboard_text(), chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=get_scorer_markup(), parse_mode="Markdown")

        # Feature 2: Shot Tracker Menu
        if dt == "menu_shot_area":
            m = InlineKeyboardMarkup(row_width=2)
            m.add(
                InlineKeyboardButton("🟢 Cover / Off Side", callback_data="shot_offside"),
                InlineKeyboardButton("🔵 Straight / Down Ground", callback_data="shot_straight")
            )
            m.add(
                InlineKeyboardButton("🟡 Mid-Wicket / Leg Side", callback_data="shot_legside"),
                InlineKeyboardButton("🔴 Behind Wicket / Gully", callback_data="shot_behind")
            )
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_home"))
            return bot.edit_message_text("🎯 **Select Shot Area (Wagon Wheel Tracker):**", chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=m, parse_mode="Markdown")

        if dt.startswith("shot_"):
            area = dt.replace("shot_", "").title()
            bot.answer_callback_query(call.id, f"Shot registered towards {area}!")
            return bot.edit_message_text(f"🎯 Last shot recorded towards: **{area}**\n\n" + get_scoreboard_text(), chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=get_scorer_markup(), parse_mode="Markdown")
        ...
        # Feature 5: Advanced DLS Target Calculator Menu
        if dt == "sc_dls":
            match["match_status"] = "DLS_APPLIED"
            if match["overs"] > 0:
                current_rr = match["runs"] / (match["balls"] / 6)
                match["target"] = int(match["runs"] + (current_rr * 3) + 1)
            bot.answer_callback_query(call.id, "DLS Method Applied!", show_alert=True)
            return bot.edit_message_text("🌧️ **DLS Par Score / Target Calculated & Adjusted!**\n\n" + get_scoreboard_text(), chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=get_scorer_markup(), parse_mode="Markdown")
        ...
        action_desc = ""
        # Standard Runs (1, 2, 3, 4, 6) & Gully Rules (1D, 2D, 3D)
        ...
        # Feature 4: Live Telegram Group Pinned/Updated Scoreboard Broadcasting
        try:
...
if __name__ == "__main__":
    ...
    print("Ultimate Pro Cricket Scorer with All 5 Features is running 24/7...")
