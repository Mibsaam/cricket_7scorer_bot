...
def create_blank_match():
    return {
        ...
        "squads": {},
        "match_innings_data": {
            1: {"team": "Team A", "batting": {}, "bowling": {}, "extras": {"w": 0, "nb": 0, "b": 0, "lb": 0, "total": 0}, "fow": []},
            2: {"team": "Team B", "batting": {}, "bowling": {}, "extras": {"w": 0, "nb": 0, "b": 0, "lb": 0, "total": 0}, "fow": []}
        },
        "career_db": {},
        "match_archives": []
    }
...
def safe_edit_message(text, chat_id, message_id, reply_markup=None):
    try:
        ...
        if "message is not modified" not in str(e):
            print(f"Edit Message Log: {e}")

# ================= CRICBUZZ VISUAL DISPLAY (VARIATION 1 LOCKED) =================
def get_large_scoreboard_text():
    ...
    st_sr = (st_b['runs'] / st_b['balls'] * 100) if st_b['balls'] > 0 else 0.0
    ...
    return (
        ...
        f"║                                                      ║\n"
        f"║  🏏 ┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓   ║\n"
        f"║     ┃   🔴 𝐒 𝐂 𝐎 𝐑 𝐄  :   【  {match['runs']}  /  {match['wickets']}  】     ┃   ║\n"
        f"║     ┃   ⏳ 𝐎 𝐕 𝐄 𝐑 𝐒  :   【  {match['overs']}  /  {match['total_match_overs']}.0  】   ┃   ║\n"
        f"║     ┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛   ║\n"
        f"║                                                      ║\n"
        ...
        f"║ 🏏 ★ 𝐒𝐓𝐑𝐈𝐊𝐄𝐑                                         ║\n"
        f"║    👉 **{match['striker']}** 👈\n"
        f"║    💥 `{st_b['runs']}* ({st_b['balls']}b)` │ 4s: `{st_b['fours']}` │ 6s: `{st_b['sixes']}` │ SR: `{st_sr:.1f}`\n"
        f"║                                                      ║\n"
        f"║ 🏃 NON-STRIKER : **{match['non_striker']}** `{nst_b['runs']} ({nst_b['balls']}b)`\n"
        f"║ ⚾ CURRENT BOWLER: **{match['bowler']}** (`{b_st['wickets']}/{b_st['runs']}` in `{b_ov}` ov)\n"
        f"║ 🤝 PARTNERSHIP : `{match['partnership_runs']} runs ({match['partnership_balls']} balls)`\n"
        f"║ 🎞️ OVER LOG    : `[ {rec_balls} ]`\n"
        f"║ 🧤 WK: `{match['wicketkeeper']}` │ 👑 CAP: `{match['captain']}`\n"
        f"╠══════════════════════════════════════════════════════╣\n"
        f"║ 🎙️ EVENT: {match['last_event_ticker']}\n"
        f"╚══════════════════════════════════════════════════════╝"
    )
...
def generate_detailed_scorecard_text(match_data=None):
    ...
    for inn in [1, 2]:
        ...
        if not batting_dict and inn == d["current_inning"]:
            batting_dict = {
                d["striker"]: {"runs": 0, "balls": 0, "fours": 0, "sixes": 0, "status": "Not Out"},
                d["non_striker"]: {"runs": 0, "balls": 0, "fours": 0, "sixes": 0, "status": "Not Out"}
            }
        ...
        if not bowling_dict and inn == d["current_inning"]:
            bowling_dict = {d["bowler"]: {"balls": 0, "runs": 0, "wickets": 0, "maidens": 0}}
            
        for b, st in bowling_dict.items():
            if b.startswith("Select"): continue
...
def get_scorer_keyboard(uid):
    ...
    m.add(
        InlineKeyboardButton("🌧️ DLS Target", callback_data="menu_dls_reduction"),
...
def sync_pinned_card(cid):
    try:
        ...
        if match.get("pinned_message_id") and match.get("pinned_chat_id") == cid:
            safe_edit_message(txt, chat_id=cid, message_id=match["pinned_message_id"])
    except:
        pass
...
def check_match_completion(cid):
    ...
    if match["current_inning"] == 1:
        if match["overs"] >= match["total_match_overs"] or match["wickets"] >= limit_w:
            match["target"] = max(1, match["runs"] + 1)
            txt = f"🏁 **INNINGS 1 COMPLETE!**\n**{match['batting_team']}** scored `{match['runs']}/{match['wickets']}` in `{match['overs']}` ov.\n🎯 **Target for {match['bowling_team']}:** `{match['target']}` runs."
            broadcast_commentary(cid, txt)
            m = InlineKeyboardMarkup(row_width=1)
            m.add(InlineKeyboardButton("🏏 Start 2nd Innings & Select Openers", callback_data="act_switch_innings"))
            bot.send_message(cid, "👉 Click below to deploy 2nd Innings:", reply_markup=m, parse_mode="Markdown")
            return

    elif match["current_inning"] == 2:
        if match["runs"] >= match["target"]:
            ...
            txt = f"🏆 🎊 **CHAMPIONS!** **{winner}** WON Match {match['series_current_match_num']} by **{w_left} wickets** (with {b_left} balls remaining)! 🥇"
            broadcast_commentary(cid, txt)
            archive_match()
            ...
            if margin == 0:
                ...
                match["series_tally"][winner] = match["series_tally"].get(winner, 0) + 1
                txt = f"🏆 🎊 **VICTORY!** **{winner}** WON Match {match['series_current_match_num']} by **{margin} runs**! 🥇"
                broadcast_commentary(cid, txt)
            archive_match()
...
def register_legal_ball(cid, legal=True, ball_tag="0"):
    if legal:
        ...
        # CHECK INNINGS OVER LIMIT FIRST
        if match["overs"] >= match["total_match_overs"] or match["wickets"] >= match["max_wickets_limit"] or (match["current_inning"] == 2 and match["runs"] >= match["target"]):
            check_match_completion(cid)
            return

        # OVER ROTATION POPUP ONLY IF OVERS REMAIN
...
def start_wizard_openers_step(cid, mid):
    match["awaiting_action"] = "wiz_type_striker"
    m = InlineKeyboardMarkup(row_width=2)
...
def master_action_handler(c):
    try:
        ...
        if match.get("match_status") == "Finished" and dt.startswith("act_run_"):
            return bot.answer_callback_query(c.id, "⚠️ Match is finished! Scoring locked.", show_alert=True)
        ...
        if dt in ["wiz_mode_real", "wiz_mode_practice"]:
            ...
            return safe_edit_message("📌 **Step 1.2:** Select **Series Format:**", chat_id=cid, message_id=mid, reply_markup=m)
        ...
        if dt.startswith("wiz_add_p_"):
            ...
            return safe_edit_message(f"✍️ **{team_n}** ke player ka naam type karke send karein:", chat_id=cid, message_id=mid)
        ...
        if dt.startswith("wiz_set_g_"):
            ...
            return safe_edit_message(f"🏟️ Ground: **{g_name}**\n\n📌 **Step 6:** Select **Total Match Overs:**", chat_id=cid, message_id=mid, reply_markup=m)
        ...
        # STEP 7 OPENERS PROMPTS
        if dt in ["wiz_type_striker", "type_new_striker"]:
            match["awaiting_action"] = "wiz_type_striker"
            return safe_edit_message(f"✍️ **Striker ({match['batting_team']})** ka naam type karke send karein:", chat_id=cid, message_id=mid)

        if dt in ["wiz_type_nonstriker", "type_new_nonstriker"]:
            match["awaiting_action"] = "wiz_type_nonstriker"
            return safe_edit_message(f"✍️ **Non-Striker (Runner)** ka naam type karke send karein:", chat_id=cid, message_id=mid)

        if dt in ["wiz_type_bowler", "type_new_bowler"]:
            match["awaiting_action"] = "wiz_type_bowler"
        ...
        if dt.startswith("wiz_str_"):
            ...
            return safe_edit_message(f"👤 Striker: **{p_name}**\n\n📌 **Step 7.2:** Select/Type **Non-Striker (Runner):**", chat_id=cid, message_id=mid, reply_markup=m)
        ...
        if dt.startswith("wiz_nstr_"):
            ...
            return safe_edit_message(f"🏃 Runner: **{p_name}**\n\n📌 **Step 7.3:** Select/Type **Opening Bowler ({match['bowling_team']}):**", chat_id=cid, message_id=mid, reply_markup=m)
        ...
        if dt.startswith("exec_nb_"):
            ...
            return safe_edit_message(f"⚠️ **NO BALL (+{tot} Runs Recorded)!**\nKya agli ball par Free Hit enforce karni hai?", chat_id=cid, message_id=mid, reply_markup=m)
        ...
        if dt == "pop_injury_split":
            m = InlineKeyboardMarkup(row_width=2)
            m.add(InlineKeyboardButton("🚑 Change Batsman (Retired)", callback_data="pop_set_striker"))
            m.add(InlineKeyboardButton("🚑 Change Bowler (Injured)", callback_data="pop_set_bowler"))
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
        ...
        if dt == "pop_set_striker":
            ...
            m.add(InlineKeyboardButton("➕ Type New Batsman", callback_data="wiz_type_striker"))
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
        ...
        if dt == "pop_set_nonstriker":
            ...
            m.add(InlineKeyboardButton("➕ Type New Non-Striker", callback_data="wiz_type_nonstriker"))
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
        ...
        if dt.startswith("set_cap_"):
            p = dt.replace("set_cap_", "")
            match["captain"] = p
            save_data()
            return safe_edit_message(f"✅ Captain assigned to **{p}**!\n\n{get_large_scoreboard_text()}", chat_id=cid, message_id=mid, reply_markup=get_scorer_keyboard(uid))

        # FAST OPPONENT ENTRY HANDLERS
        ...
        if dt == "menu_dls_reduction":
            ...
            return safe_edit_message("🌧️ **DLS Rain Overs Reduction:**\nBarish ke baad match ke **Revised Total Overs** kitne hue? Type karein (e.g. 5):", chat_id=cid, message_id=mid)
        ...
        if dt == "act_switch_innings":
            if match["current_inning"] == 1:
                ...
                match.update({"runs": 0, "wickets": 0, "overs": 0.0, "balls": 0, "extras_total": 0, "extras_wides": 0, "extras_noballs": 0, "extras_byes": 0, "extras_legbyes": 0, "partnership_runs": 0, "partnership_balls": 0, "recent_balls": [], "current_over_runs": 0, "striker": "Select Striker", "non_striker": "Select Non-Striker", "bowler": "Select Bowler", "last_bowler": None, "last_event_ticker": f"Innings 2 Started! Target: {match['target']}"})
                save_data()
                return start_wizard_openers_step(cid, mid)
        ...
        if dt == "act_reset_stats_confirm":
            ...
            return safe_edit_message("⚠️ **Kya aap current match score reset karna chahte hain?**", chat_id=cid, message_id=mid, reply_markup=m)

        if dt == "act_reset_stats_exec":
            match.update({"runs": 0, "wickets": 0, "overs": 0.0, "balls": 0, "extras_total": 0, "extras_wides": 0, "extras_noballs": 0, "extras_byes": 0, "extras_legbyes": 0, "partnership_runs": 0, "partnership_balls": 0, "recent_balls": [], "current_over_runs": 0, "history": [], "match_status": "Active", "last_event_ticker": "Match stats reset clean."})
            save_data()
            return safe_edit_message(get_large_scoreboard_text(), chat_id=cid, message_id=mid, reply_markup=get_scorer_keyboard(uid))
...
def process_wicket(cid, mid, reason, uid, target_batter="striker"):
    ...
    if match["wickets"] < limit_w:
        ...
        match["last_event_ticker"] = f"🚨 WICKET! {dismissed} ({reason})!"
        ...
        m = InlineKeyboardMarkup(row_width=2)
        for p in match["squads"].get(match["batting_team"], []):
            if p != match["striker"] and p != match["non_striker"] and p != dismissed:
                m.add(InlineKeyboardButton(f"🏏 {p}", callback_data=f"sel_str_{p}" if target_batter == "striker" else f"sel_nonstr_{p}"))
        m.add(InlineKeyboardButton("➕ Type Next Batsman", callback_data="wiz_type_striker" if target_batter == "striker" else "wiz_type_nonstriker"))
        bot.send_message(cid, f"👤 **{dismissed} OUT!** Next Batsman select karein:", reply_markup=m, parse_mode="Markdown")
        if mid:
            return safe_edit_message(get_large_scoreboard_text(), chat_id=cid, message_id=mid, reply_markup=get_scorer_keyboard(uid))
...
def handle_text_inputs(msg):
    ...
    if act == "input_rename_team_val":
        ...
        try:
            ...
            match["match_innings_data"][1] = {
                ...
                "extras": {"w": 0, "nb": 0, "b": 0, "lb": 0, "total": 0}, "fow": []
            }
            match.update({"runs": 0, "wickets": 0, "overs": 0.0, "balls": 0, "extras_total": 0, "partnership_runs": 0, "partnership_balls": 0, "recent_balls": [], "history": [], "last_event_ticker": f"Innings 1: {r}/{w} ({ov}.0 ov). Target: {match['target']}"})
            save_data()
        ...
        try:
            ...
            if match["current_inning"] == 2 and orig_ov > 0:
                match["target"] = max(1, int((match["target"] * (rev_ov / orig_ov)) + 1))
    ...
    # STEP 7 DIRECT TEXT / BUTTON LISTENERS
        ...
        for p in match["squads"].get(match["batting_team"], []):
            if p != txt:
                m.add(InlineKeyboardButton(f"🏃 {p}", callback_data=f"wiz_nstr_{p}"))
        m.add(InlineKeyboardButton("➕ Type Non-Striker", callback_data="wiz_type_nonstriker"))
        bot.reply_to(msg, f"✅ Striker: **{txt}**\n\n📌 **Step 7.2:** Select/Type **Non-Striker (Runner):**\n_(Aap direct naam bhi type karke bhej sakte hain)_", reply_markup=m, parse_mode="Markdown")
        ...
        for p in match["squads"].get(match["bowling_team"], []):
            m.add(InlineKeyboardButton(f"⚾ {p}", callback_data=f"wiz_bowl_{p}"))
        m.add(InlineKeyboardButton("➕ Type Opening Bowler", callback_data="wiz_type_bowler"))
        bot.reply_to(msg, f"✅ Runner: **{txt}**\n\n📌 **Step 7.3:** Select/Type **Opening Bowler ({match['bowling_team']}):**\n_(Aap direct naam bhi type karke bhej sakte hain)_", reply_markup=m, parse_mode="Markdown")
        ...
        msg_obj = bot.send_message(msg.chat.id, get_large_scoreboard_text(), reply_markup=get_scorer_keyboard(uid), parse_mode="Markdown")
        try:
