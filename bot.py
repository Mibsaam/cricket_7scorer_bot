...
def get_large_scoreboard_text():
    ...
    return (
        f"╭──────────────────────────────────────────────────────╮\n"
        f"│  🔴 **{clean_txt(match['batting_team'])}**  :  【  **{match['runs']}  /  {match['wickets']}**  】  │  **{match['overs']}** Ov  │  𝐂𝐑𝐑: **{crr:.2f}**   │\n"
        f"╰──────────────────────────────────────────────────────╯\n\n"
        f"  🏏 ★  **{clean_txt(match['striker'])}** (On Strike)   ▶   **{st_b['runs']}** ₍{st_b_sub}₎   │  𝟒𝐬: {st_b['fours']}  𝟔𝐬: {st_b['sixes']}\n\n"
        f"  🏃    **{clean_txt(match['non_striker'])}**        ▶   **{nst_b['runs']}** ₍{nst_b_sub}₎   │  𝟒𝐬: {nst_b['fours']}  𝟔𝐬: {nst_b['sixes']}\n\n"
        f"  ⚾    **{clean_txt(match['bowler'])}**      ▶   **{b_st['wickets']} / {b_st['runs']}** ({b_ov} ov)\n\n"
        f"{bottom_box}\n"
        f"🏟️ Ground: `{clean_txt(match['ground'])}` │ 🏆 **{clean_txt(match['series_name'])}** (M {match['series_current_match_num']}/{match['series_total_matches']}){fh_tag}\n"
        f"🎙️ Event: {clean_ticker}"
    )
...
def generate_detailed_scorecard_text(match_data=None):
    ...
    out = (
        ...
        f"🏟️ Ground: `{clean_txt(d['ground'])}` │ Match Overs: `{d['total_match_overs']}.0` (Max Wkts: {d.get('max_wickets_limit', 10)})\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
    )
    ...
    for inn in [1, 2]:
        ...
        for p, st in batting_dict.items():
            ...
            out += f"• **{clean_txt(p)}**: `{st['runs']}` ({st['balls']}b) [4s:{st['fours']}, 6s:{st['sixes']}] │ SR: `{sr:.1f}` │ _{clean_txt(st['status'])}_\n"
        ...
        for b, st in bowling_dict.items():
            ...
            out += f"• **{clean_txt(b)}**: `{ov} ov` (M:{st.get('maidens',0)}) │ `{st['runs']} runs` │ `{st['wickets']} wkts` │ Econ: `{econ:.2f}`\n"
...
def check_match_completion(cid):
    ...
    if match["current_inning"] == 1:
        if match["overs"] >= match["total_match_overs"] or match["wickets"] >= limit_w:
            ...
            txt = f"🏁 **INNINGS 1 COMPLETE!**\n**{clean_txt(match['batting_team'])}** scored `{match['runs']}/{match['wickets']}` in `{match['overs']}` ov.\n🎯 **Target for {clean_txt(match['bowling_team'])}:** `{match['target']}` runs."
        ...
        if match["runs"] >= match["target"]:
            ...
            txt = f"🏆 🎊 **CHAMPIONS!** **{clean_txt(winner)}** WON Match {match['series_current_match_num']} by **{w_left} wickets** (with {b_left} balls remaining)! 🥇"
            broadcast_commentary(cid, txt)
            archive_match()
            ...
            if margin == 0:
                ...
                match["series_tally"][winner] = match["series_tally"].get(winner, 0) + 1
                txt = f"🏆 🎊 **VICTORY!** **{clean_txt(winner)}** WON Match {match['series_current_match_num']} by **{margin} runs**! 🥇"
                broadcast_commentary(cid, txt)
            archive_match()
...
def register_legal_ball(cid, legal=True, ball_tag="0", runs_on_ball=0):
    if legal:
        ...
        if rem_b == 0 and match["balls"] > 0:
            ...
            # Rotate strike on over completion if last ball run was even
            if runs_on_ball % 2 == 0:
...
def master_action_handler(c):
    try:
        ...
        if match.get("match_status") == "Finished" and (dt.startswith("act_run_") or dt.startswith("wkt_")):
            return safe_send_message(cid, "⚠️ Match is finished! Scoring locked.")
...
def process_wicket(cid, mid, reason, uid, target_batter="striker"):
    ...
    if match["wickets"] < limit_w:
        ...
        fow_txt = f"{match['runs']}/{match['wickets']} ({clean_txt(dismissed)}, {match['overs']} ov)"
        ...
        safe_send_message(cid, f"👤 **{clean_txt(dismissed)} OUT!** Next Batsman select karein:", reply_markup=m)
        if mid:
            return safe_edit_message(get_large_scoreboard_text(), chat_id=cid, message_id=mid, reply_markup=get_scorer_keyboard(uid))
...
def handle_text_inputs(msg):
    ...
    try:
        if act == "wiz_type_series_name":
            ...
            m.add(InlineKeyboardButton("➡️ Continue to Ground & Overs", callback_data="wiz_ground_step"))
            safe_send_message(msg.chat.id, f"✅ Added **{txt}** to **{t_target}**!", reply_markup=m)
            ...
            try:
                ...
                save_data()
                sync_pinned_card(msg.chat.id)
                safe_send_message(msg.chat.id, f"✅ **DLS Applied!** New Total Overs: `{rev_ov}.0` │ Revised Target: `{match['target']}` Runs\n\n{get_large_scoreboard_text()}", reply_markup=get_scorer_keyboard(uid))
            except Exception:
            ...
            for p in match["squads"].get(match["batting_team"], []):
                if p != txt:
                    m.add(InlineKeyboardButton(f"🏃 {p}", callback_data=f"wiz_nstr_{p}"))
            m.add(InlineKeyboardButton("➕ Type Non-Striker", callback_data="wiz_type_nonstriker"))
            safe_send_message(msg.chat.id, f"✅ Striker: **{txt}**\n\n📌 **Step 7.2:** Select/Type **Non-Striker (Runner):**\n_(Aap direct naam bhi type karke bhej sakte hain)_", reply_markup=m)
