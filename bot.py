...
def get_large_scoreboard_text():
    ...
    series_info = f"Match {match['series_current_match_num']} of {match['series_total_matches']}"
    
    return (
        f"╔══════════════════════════════════════╗\n"
        f"║ 🏆 **{match['series_name']}** ({series_info})\n"
...
def generate_detailed_scorecard_text(match_data=None):
    ...
    out = (
        ...
        f"🏆 **{d['series_name']}** │ Match {d.get('series_current_match_num', 1)}/{d.get('series_total_matches', 1)}\n"
...
def get_scorer_keyboard(uid):
    ...
    m.add(
        InlineKeyboardButton("➕ Extend / Add Match", callback_data="menu_extend_series"),
...
def handle_start_wizard(msg):
    ...
    bot.reply_to(msg, "🏏 **PRO CRICKET ENGINE - INTERACTIVE SETUP**\n\n📌 **Step 1:** Select Match / Tournament Mode:", reply_markup=m, parse_mode="Markdown")
...
def check_match_completion(cid):
    ...
    if match["current_inning"] == 2:
        if match["runs"] >= match["target"]:
            ...
            b_left = max(0, (match["total_match_overs"] * 6) - match["balls"])
            txt = f"🏆 🎊 **CHAMPIONS!** **{match['batting_team']}** WON Match {match['series_current_match_num']} by **{w_left} wickets** (with {b_left} balls remaining)! 🥇"
            ...
            if margin == 0:
                ...
                txt = f"🏆 🎊 **VICTORY!** **{match['bowling_team']}** WON Match {match['series_current_match_num']} by **{margin} runs**! 🥇"
                broadcast_commentary(cid, txt)
            archive_match()
...
def start_wizard_squad_step(cid, mid):
    ...
    txt = f"📌 **Step 4: Manage Squads for {t1} & {t2}:**\n(Players add karein ya direct continue karein)"
    if mid:
        bot.edit_message_text(txt, chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")
    else:
...
def start_wizard_openers_step(cid, mid):
    ...
    txt = f"📌 **Step 7.1:** Select **Opening Striker ({match['batting_team']}):**"
    if mid:
        bot.edit_message_text(txt, chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")
    else:
        bot.send_message(cid, txt, reply_markup=m, parse_mode="Markdown")
    ...
    try:
        ...
        if dt == "view_archives":
            ...
            for arc in match["match_archives"][-8:]:
                m.add(InlineKeyboardButton(f"📁 #{arc['match_id']}: {arc['teams']} (Match {arc.get('series_current_match_num', 1)})", callback_data=f"arc_{arc['match_id']}"))
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
            return bot.edit_message_text("📜 **MATCH ARCHIVE VAULT:**", chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")
        ...
        if dt == "view_mom":
            ...
            for p, st in match["career_db"].items():
                pts = (st["runs"] * 1.5) + (st["fours"] * 2.0) + (st["sixes"] * 4.0) + (st["wickets"] * 35) + (st["catches"] * 15) - (st["drops"] * 5)
                if pts > mos_pts:
                    mos_pts, mos_p = pts, p

            res_txt = (
                f"⭐ **MAN OF THE MATCH:** `{best_p}` (Impact: `{max_pts:.1f}` pts)\n"
                f"🏆 Priority awarded to winning team impact!\n\n"
                f"🎖️ **SERIES / TOURNAMENT IMPACT LEADER (MoS):**\n"
                f"👉 `{mos_p}` with `{mos_pts:.1f}` tournament points across all matches!"
            )
            return bot.send_message(cid, res_txt, parse_mode="Markdown")
        ...
        # ================= STEP-BY-STEP WIZARD ROUTING =================
        if dt in ["wiz_mode_real", "wiz_mode_practice"]:
            match["is_practice_mode"] = (dt == "wiz_mode_practice")
            match["tournament_mode"] = False
            m = InlineKeyboardMarkup(row_width=2)
            m.add(InlineKeyboardButton("✍️ Custom / Manual Series Length", callback_data="wiz_series_custom"))
            ...
            return bot.edit_message_text("📌 **Step 1.2:** Select **Series Format:**", chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")
        ...
        if dt == "wiz_mode_tournament":
            ...
            return bot.edit_message_text("🏟️ **Multi-Team Tournament Mode:**\nTournament mein kitni teams hongi?", chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")

        if dt.startswith("tour_teams_"):
            val = dt.replace("tour_teams_", "")
            if val == "custom":
                match["awaiting_action"] = "input_tour_custom_teams_count"
                return bot.edit_message_text("✍️ Tournament mein kitni teams hongi number type karein:", chat_id=cid, message_id=mid, parse_mode="Markdown")
            else:
                match["temp_data"]["tour_teams_needed"] = int(val)
                match["tournament_teams"] = []
                match["awaiting_action"] = "input_tour_team_name"
                return bot.edit_message_text(f"✍️ Team 1 ka naam type karke send karein (1/{val}):", chat_id=cid, message_id=mid, parse_mode="Markdown")

        if dt == "wiz_series_custom":
            match["awaiting_action"] = "input_series_custom_matches"
            return bot.edit_message_text("✍️ Series mein total **kitne matches** honge type karein (e.g. 2, 4, 6):", chat_id=cid, message_id=mid, parse_mode="Markdown")
        ...
        if dt.startswith("wiz_series_"):
            ...
            return bot.edit_message_text("📌 **Step 2:** Select **Team 1** (or Create New):", chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")

        if dt == "wiz_create_team":
            match["awaiting_action"] = "wiz_input_team"
            return bot.edit_message_text("✍️ Nayi **Team ka Naam** type karke send karein:", chat_id=cid, message_id=mid, parse_mode="Markdown")
        ...
        if dt.startswith("wiz_t1_"):
            ...
            return bot.edit_message_text(f"📌 **Step 2.2:** Team 1: **{t1}**\nAb **Team 2** select karein:", chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")

        if dt == "wiz_create_team_2":
            match["awaiting_action"] = "wiz_input_team_2"
            return bot.edit_message_text("✍️ Dusri **Team ka Naam** type karke send karein:", chat_id=cid, message_id=mid, parse_mode="Markdown")
        ...
        if dt.startswith("wiz_t2_"):
            ...
            return bot.edit_message_text(f"📌 **Step 3:** Toss Setup for **{t1} vs {t2}**:", chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")
        ...
        if dt == "wiz_toss_manual":
            ...
            return bot.edit_message_text("📌 **Step 3.2:** Kaunsi team **Pehle Batting** karegi?", chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")
        ...
        if dt.startswith("wiz_add_p_"):
            team_n = dt.replace("wiz_add_p_", "")
            match["temp_data"]["target_team"] = team_n
            match["awaiting_action"] = "input_squad_player"
            return bot.edit_message_text(f"✍️ **{team_n}** ke player ka naam type karke send karein:", chat_id=cid, message_id=mid, parse_mode="Markdown")
        ...
        if dt == "wiz_ground_step":
            ...
            return bot.edit_message_text("📌 **Step 5:** Select **Match Ground:**", chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")

        if dt.startswith("wiz_set_g_"):
            g_name = dt.replace("wiz_set_g_", "")
            match["ground"] = g_name
            m = InlineKeyboardMarkup(row_width=3)
            ...
            return bot.edit_message_text(f"🏟️ Ground: **{g_name}**\n\n📌 **Step 6:** Select **Total Match Overs:**", chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")
        ...
        if dt == "wiz_type_ground":
            match["awaiting_action"] = "wiz_type_ground"
            return bot.edit_message_text("✍️ Naya Ground Name type karke send karein:", chat_id=cid, message_id=mid, parse_mode="Markdown")

        if dt == "wiz_type_overs":
            match["awaiting_action"] = "wiz_type_overs"
            return bot.edit_message_text("✍️ Match kitne overs ka hai number type karein:", chat_id=cid, message_id=mid, parse_mode="Markdown")
        ...
        if dt.startswith("wiz_str_"):
            ...
            return bot.edit_message_text(f"👤 Striker: **{p_name}**\n\n📌 **Step 7.2:** Select **Non-Striker (Runner):**", chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")
        ...
        if dt.startswith("wiz_nstr_"):
            ...
            return bot.edit_message_text(f"🏃 Runner: **{p_name}**\n\n📌 **Step 7.3:** Select **Opening Bowler ({match['bowling_team']}):**", chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")
        ...
        if dt == "menu_custom_runs":
            ...
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
            return bot.edit_message_text("✍️ **Select Custom / Overthrow Runs:**", chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")
        ...
        if dt == "type_custom_runs_val":
            ...
            return bot.edit_message_text("✍️ Kitne runs bane type karke send karein:", chat_id=cid, message_id=mid, parse_mode="Markdown")
        ...
        if dt == "menu_wide":
            ...
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
            return bot.edit_message_text("⚡ **Select Wide Deliveries:**", chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")
        ...
        if dt == "menu_noball":
            ...
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
            return bot.edit_message_text("⚠️ **Select No Ball Deliveries (+1 Auto):**", chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")
        ...
        if dt.startswith("exec_nb_"):
            ...
            return bot.edit_message_text(f"⚠️ **NO BALL (+{tot} Runs Recorded)!**\nKya agli ball par Free Hit enforce karni hai?", chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")
        ...
        if dt == "menu_byes":
            ...
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
            return bot.edit_message_text("🏃 **Dynamic Byes / Leg Byes Menu:**", chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")
        ...
        if dt == "menu_wicket":
            ...
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
            return bot.edit_message_text("❌ **Select Dismissal Type:**", chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")
        ...
        if dt in ["wkt_bowled", "wkt_stumped", "wkt_runout"]:
            ...
            return process_wicket(cid, mid, dt.replace("wkt_", "").capitalize(), uid)
        ...
        if dt == "wkt_caught_menu":
            ...
            return bot.edit_message_text(f"🙌 **Who took the catch ({match['bowling_team']})?**", chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")

        if dt == "wkt_caught_quick":
            return process_wicket(cid, mid, "Caught", uid)
        ...
        if dt.startswith("catch_by_"):
            ...
            return process_wicket(cid, mid, f"c {f_name} b {match['bowler']}", uid)
        ...
        if dt == "act_drop_catch":
            ...
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
            return bot.edit_message_text("😱 **Who dropped the catch?**", chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")
        ...
        if dt == "pop_set_striker":
            ...
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
            return bot.edit_message_text("👤 **Select Striker:**", chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")
        ...
        if dt == "pop_set_nonstriker":
            ...
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
            return bot.edit_message_text("🏃 **Select Non-Striker (Runner):**", chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")
        ...
        if dt == "pop_set_bowler":
            ...
            m.add(InlineKeyboardButton("➕ Type New Bowler", callback_data="type_new_bowler"))
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
            return bot.edit_message_text("⚾ **Select Bowler:**", chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")
        ...
        if dt == "menu_squads_master":
            ...
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
            return bot.edit_message_text("👥 **Squad & Teams Master Control:**", chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")
        ...
        if dt == "menu_rename_team":
            ...
            return bot.edit_message_text("✏️ Kis team ka naam rename karna hai?", chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")

        if dt.startswith("ren_t_"):
            t_name = dt.replace("ren_t_", "")
            match["temp_data"]["rename_target_team"] = t_name
            match["awaiting_action"] = "input_rename_team_val"
            return bot.edit_message_text(f"✍️ **{t_name}** ka naya naam type karke send karein:", chat_id=cid, message_id=mid, parse_mode="Markdown")
        ...
        if dt == "menu_delete_team":
            ...
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="menu_squads_master"))
            return bot.edit_message_text("🗑️ Kaunsi team delete karni hai?", chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")

        if dt.startswith("del_t_"):
            t_name = dt.replace("del_t_", "")
            if t_name in match.get("teams", []): match["teams"].remove(t_name)
            if t_name in match.get("squads", {}): del match["squads"][t_name]
            save_data()
            return bot.edit_message_text(f"✅ Team **{t_name}** deleted successfully!", chat_id=cid, message_id=mid, reply_markup=get_scorer_keyboard(uid), parse_mode="Markdown")

        if dt.startswith("sq_view_"):
            t_name = dt.replace("sq_view_", "")
            p_list = match["squads"].get(t_name, [])
            p_txt = "\n".join([f"• {p}" for p in p_list]) if p_list else "Koi player add nahi hai!"
            m = InlineKeyboardMarkup(row_width=2)
            m.add(InlineKeyboardButton(f"➕ Add Player to {t_name}", callback_data=f"wiz_add_p_{t_name}"))
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="menu_squads_master"))
            return bot.edit_message_text(f"👥 **Squad: {t_name}**\n\n{p_txt}", chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")
        ...
        if dt == "menu_scorers_admin":
            ...
            return bot.edit_message_text("🛡️ **Official Scorer Management:**", chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")

        if dt == "pop_add_scorer":
            match["awaiting_action"] = "input_add_scorer"
            return bot.edit_message_text("✍️ Naye Scorer ki **Telegram User ID** type karke bhejein:", chat_id=cid, message_id=mid, parse_mode="Markdown")
        ...
        if dt == "menu_edit_match":
            ...
            m.add(InlineKeyboardButton("⬅️ Back", callback_data="back_main"))
            return bot.edit_message_text("✏️ **Live Match Data Correction:**", chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")

        if dt == "edit_tot_runs":
            match["awaiting_action"] = "input_edit_runs"
            return bot.edit_message_text("✍️ Sahi **Total Runs** type karke send karein:", chat_id=cid, message_id=mid, parse_mode="Markdown")

        if dt == "edit_tot_wkts":
            match["awaiting_action"] = "input_edit_wkts"
            return bot.edit_message_text("✍️ Sahi **Total Wickets** type karke send karein:", chat_id=cid, message_id=mid, parse_mode="Markdown")

        if dt == "edit_tot_target":
            match["awaiting_action"] = "input_edit_target"
            return bot.edit_message_text("✍️ Naya **Target** type karke send karein:", chat_id=cid, message_id=mid, parse_mode="Markdown")

        # DLS REDUCTION
        if dt == "menu_dls_reduction":
            match["awaiting_action"] = "input_dls_revised_overs"
            return bot.edit_message_text("🌧️ **DLS Rain Overs Reduction:**\nBarish ke baad match ke **Revised Total Overs** kitne hue? Type karein (e.g. 5):", chat_id=cid, message_id=mid, parse_mode="Markdown")
        ...
        if dt == "act_reset_stats_confirm":
            ...
            return bot.edit_message_text("⚠️ **Kya aap current match score reset karna chahte hain?**", chat_id=cid, message_id=mid, reply_markup=m, parse_mode="Markdown")
...
def handle_text_inputs(msg):
    ...
    if act == "input_rename_team_val":
        ...
        if target_t in match.get("teams", []):
            ...
            save_data()
            bot.reply_to(msg, f"✅ Team **{target_t}** successfully renamed to **{txt}**!", reply_markup=get_scorer_keyboard(uid), parse_mode="Markdown")
        else:
            bot.reply_to(msg, "❌ Team nahi mili!")
        ...
        try:
            ...
            for t in match.get("teams", []):
                m.add(InlineKeyboardButton(f"📁 {t}", callback_data=f"wiz_t1_{t}"))
            m.add(InlineKeyboardButton("➕ Create Team", callback_data="wiz_create_team"))
            bot.reply_to(msg, f"✅ Series locked to **{m_cnt} Matches**!\n\n📌 **Step 2:** Select **Team 1**:", reply_markup=m, parse_mode="Markdown")
        except:
            bot.reply_to(msg, "❌ Invalid number of matches!")
        ...
        try:
            ...
            match["tournament_teams"] = []
            match["awaiting_action"] = "input_tour_team_name"
            bot.reply_to(msg, f"✍️ Team 1 ka naam type karke send karein (1/{cnt}):")
        except:
            bot.reply_to(msg, "❌ Invalid number of teams!")
